...
static TSCvector* boundaryStack;
...
static Boundary* allocBoundary(BoundaryEdge* g, int l, int zs, int os)
{
...
static void pushBoundary(Boundary* boundary)
{
  boundaryStack->addElement(boundary);
}

static Boundary* popBoundary()
{
  Boundary* boundary = (Boundary*)boundaryStack->lastElement();
  boundaryStack->removeLastElement();
  return boundary;
}

static int hasMoreBoundaries()
{
  return (boundaryStack->size() != 0);
}

static Boundary* getBoundary(int i)
{
  return (Boundary*)boundaryStack->elementAt(i);
}

static Boundary* removeBoundary(int i)
{
  Boundary* boundary = (Boundary*)boundaryStack->elementAt(i);
  boundaryStack->removeElementAt(i);
  return boundary;
}

static int numberBoundaries()
{
  return boundaryStack->size();
}
...
static BoundaryEdge* best_gate(BoundaryEdge* gate, int zero_slots, int one_slots)
{
  if (zero_slots)
  {
    return best_gate_zero(gate);
  }
  else if (one_slots)
  {
    return best_gate_one(gate);
  }
  else
  {
    return gate;
...
static int decompressWithZeroSlots(PSreader_oocc* ps, Boundary* boundary)
{
  int* across;
  ...
  if (slide->border)
  {
    boundary->gate = slide->boundary_prev;
...
static int decompress(PSreader_oocc* ps)
{
  int has_free_vertex;
  int across[3];
  ...
  while (true)
  {
    ...
    gate = best_gate(gate, zero_slots, one_slots);
    ...
    if ( has_free_vertex )
    {
      ...
      if (codec->readAddOrSplitMerge() == 1) // split or merge
      {
        ...
        if (codec->readSplitOrMerge() == 0) // split
        {
          ...
          if (direction)
          {
            boundary = allocBoundary(slideL, boundary_length - offset - 1, (slideL->slots == 0)+(glue->slots == 0), one_slots+((glue->slots == 1)&&(glue->border == 0))+((slideL->slots == 1)&&(slideL->border == 0))-split_one_slots-(slideL->slots == 0));
            if (!boundary->zero_slots)
            {
              pushBoundary(boundary);
              boundary = 0;
            }
            zero_slots = (slideR->slots == 0)+(gate->slots == 0);  // zero_slots of the active boundary
          ...
          {
            boundary = allocBoundary(slideR, boundary_length - offset - 1, (slideR->slots == 0)+(gate->slots == 0), one_slots-split_one_slots-(slideL->slots == 0)+((slideR->slots == 1)&&(slideR->border == 0)));
            if (!boundary->zero_slots)
            {
              pushBoundary(boundary);
              boundary = 0;
            }
            zero_slots = (slideL->slots == 0)+(glue->slots == 0); // zero_slots of the active boundary
            one_slots = split_one_slots+((glue->slots == 1)&&(glue->border == 0))+((slideL->slots == 1)&&(slideL->border == 0));
            boundary_length = offset + 2;
            gate = slideL;
          }
        }
...
bool PSreader_oocc::open(const char* file_name)
{
...
bool PSreader_oocc::open(FILE* fp)
{
  int i;
  int elem_count;
  char *elem_name;
...
PSevent PSreader_oocc::read_triangle()
{
  if (f_count == nfaces)
  {
    return PS_EOF;
...
void PSreader_oocc::set_vdata(const void* data, int i)
{
  (*(set_v_data[i])) = data;
}

void* PSreader_oocc::get_vdata(int i) const
{
  return (void*)get_v_data[i];
}

void PSreader_oocc::set_edata(const void* data, int i)
{
  (*(set_e_data[i])) = data;
}

void* PSreader_oocc::get_edata(int i) const
{
  return (void*)get_e_data[i];
}

void PSreader_oocc::close()
{
