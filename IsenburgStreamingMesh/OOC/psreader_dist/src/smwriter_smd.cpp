...
void SMwriter_smd::add_comment(const char* comment)
{
  fprintf(stderr,"WARNING: add_comments not yet implemented\n");
}

void SMwriter_smd::set_nverts(int nverts)
{
  this->nverts = nverts;
}

void SMwriter_smd::set_nfaces(int nfaces)
{
  this->nfaces = nfaces;
}

void SMwriter_smd::set_boundingbox(const float* bb_min_f, const float* bb_max_f)
{
  if (bb_min_f)
  {
  ...
  {
    fprintf(stderr, "WARNING: bb_min_f pointer was null.\n");
  }
  if (bb_max_f)
  {
  ...
  {
    fprintf(stderr, "WARNING: bb_max_f pointer was null.\n");
  }
}
...
bool SMwriter_smd::open(FILE* file, int bits, int delay)
{
  // write version
  if (file)
  {
